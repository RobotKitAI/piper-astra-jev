...
The recordings are not in the repository itself. Those of runs 3 and 4 are attached to the
[recordings release](https://github.com/RobotKitAI/piper-astra-jev/releases/tag/recordings)
as zips (without the depth colour-map video); unzip them into `out/recordings`.
`piper_llm/record.py` writes your own runs to the same place. From the
repository root:
